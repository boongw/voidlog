...
// tied to any dragon's theme, so they get a neutral gray instead.
const PURIFICATION_COLOR = '#2e2845';

export function phaseColor(order: number, name?: string): string {
    if (name?.startsWith('Purification')) return PURIFICATION_COLOR;
...
export function PhaseBadge({name, order}: Readonly<{ name: string; order: number }>) {
